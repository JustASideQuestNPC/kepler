...
/** @type {number[]} */
const VALID_KEY_CODES = Object.values(Key);
...
class KInput {
  ...
  /**
   * If `true`, pressing F12 will still open the debug console in your browser.
   * @static
   * @type {boolean}
   */
  static F12_PASSTHROUGH_ENABLED = true;
   ...
   * @property {boolean} chord If true, the action can only activate when every
   *    key or button bound to it is pressed at the same time. Otherwise, the
   *    action can activate whenever at least one key or button bound to it is
   *    pressed. The default chord setting is `false`.
   ...
   * @property {function(boolean[]): boolean} boundKeyPressed Internal function
   *    that determines whether at least one key or button bound to the input is
   *    pressed (unless the action is a chord, in which case it determines
   *    whether all of them are pressed).
   * @property {function(boolean[]): void} update Internal function that updates
   *    whether the action is active based on its activation mode and what keys
   *    or buttons are currently pressed.
  ...
  /**
   * Creates a new KInput object and defines `keyPressed`, `keyReleased`,
   * `mousePressed`, and `mouseReleased` for it if they aren't already defined.
   * Use this to create KInput objects, **not** `new KInput()`!
   * @static
   * @method
   ...
   * @returns {KInput} The KInput object created and referenced in the input
   *    listeners.
   */
  static makeNew(sketch) {
    // use the key to create a new KInput without throwing an exception
    let obj = new KInput(KInput.#CONSTRUCTOR_KEY);

    // auto-define any undefined input listeners
    if (typeof sketch.keyPressed !== "function") {
      sketch.keyPressed = () => {
        obj.pressKey(sketch.keyCode);
        return (KInput.F12_PASSTHROUGH_ENABLED && sketch.keyCode === Key.F12);
      }
    }
  ...
  constructor(constructorKey) {
    if (constructorKey !== KInput.#CONSTRUCTOR_KEY) throw new Error('KInput ' +
        'objects cannot be constructed with "new KInput" - use ' + 
   ...
   * @param {Key[] | Key} action.keys - All keys and/or mouse buttons that can
  ...
  addAction({name, keys, mode="continuous", chord=false, callback=(()=>{})}) {
    ...
    if (keys == null) throw new TypeError(`The input action "${name}" has no ` +
        `keys or mouse buttons assigned to it!`);

    // convert single keys into an array, then make sure everything is a key
  ...
  /**
   * Updates the internal state when a key is pressed. If you're defining input
   * listeners yourself, this should be called in `keyPressed` as
   * `pressKey(keyCode)`.
   * @method
  ...
  /**
   * Updates the internal state when a key is released. If you're defining input
   * listeners yourself, this should be called in `keyReleased` as
   * `releaseKey(keyCode)`.
   * @method
   * @param {number} code - The key that triggered the listener; should be
   *    `keyCode`.
   ...
   * defining input listeners yourself, this should be called in `mousePressed`
   * as `pressMouse(mouseButton)`.
   * @method
   * @param {("left" | "right" | "center")} button - The mouse button that
   *    triggered the listener; should be `mouseButton`.
   */
  pressMouse(button) {
    if (button === "left") this.#keyStates[Key.LEFT_MOUSE] = true;
    else if (button === "right") this.#keyStates[Key.RIGHT_MOUSE] = true;
    else this.#keyStates[Key.MIDDLE_MOUSE] = true;
  }

  /**
   * Updates the internal state when a mouse button is released. If you're
   * defining input listeners yourself, this should be called in `mouseReleased`
   * as `releaseMouse(mouseButton)`.
   * @method
   * @param {("left" | "right" | "center")} button - The mouse button that
   *    triggered the listener; should be `mouseButton`.
   */
  releaseMouse(button) {
    if (button === "left") this.#keyStates[Key.LEFT_MOUSE] = false;
    else if (button === "right") this.#keyStates[Key.RIGHT_MOUSE] = false;
    else this.#keyStates[Key.MIDDLE_MOUSE] = false;
  }
}